			...
			case 'membership':
				...
				while ($row = $db->sql_fetchrow($result))
				{
					...
					$template->assign_block_vars($block, array(
						...
						'GROUP_DESC'	=> ($row['group_type'] <> GROUP_SPECIAL) ? generate_text_for_display($row['group_desc'], $row['group_desc_bitfield'], $row['group_desc_options']) : $user->lang['GROUP_IS_SPECIAL'],
				...
				$sql = 'SELECT group_id, group_name, group_colour, group_desc, group_desc_bitfield, group_desc_options, group_type, group_founder_manage
					FROM ' . GROUPS_TABLE . '
			...
			case 'manage':
				...
				if ($group_id)
				{
					...
					$template->assign_vars(array(
						...
						'GROUP_DESC_DISP'		=> generate_text_for_display($group_row['group_desc'], $group_row['group_desc_bitfield'], $group_row['group_desc_options']),
						'GROUP_TYPE'			=> $group_row['group_type'],
				...
				switch ($action)
				{
					case 'edit':
						...
						{
							$group_desc_data = generate_text_for_edit($group_row['group_desc'],  $group_row['group_desc_options']);
					...
					default:
						...
						$sql = 'SELECT g.group_id, g.group_name, g.group_colour, g.group_desc, g.group_desc_bitfield, g.group_desc_options, g.group_type, ug.group_leader
							FROM ' . GROUPS_TABLE . ' g, ' . USER_GROUP_TABLE . ' ug
							WHERE ug.user_id = ' . $user->data['user_id'] . '
								AND g.group_id = ug.group_id
								AND ug.group_leader = 1
						...
						while ($value = $db->sql_fetchrow($result))
						{
							$template->assign_block_vars('leader', array(
								...
								'GROUP_DESC'	=> generate_text_for_display($value['group_desc'], $value['group_desc_bitfield'], $value['group_desc_options']),
