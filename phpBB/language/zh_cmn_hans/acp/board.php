...
$lang = array_merge($lang, array(
	...
	'DEFAULT_STYLE'					=> '默认风格',
	'DISABLE_BOARD'					=> '关闭论坛',
	'DISABLE_BOARD_EXPLAIN'			=> '这将让非管理员和版主的用户无法使用论坛. 您可以输入一段简单的文字描述原因(255个字符以内).',
...
$lang = array_merge($lang, array(
	...
	'ALLOW_PM_ATTACHMENTS'		=> '在消息中允许附件',
	'ALLOW_PM_REPORT'			=> '允许用户举报不良站内消息',
	'ALLOW_PM_REPORT_EXPLAIN'	=> '启用后, 用户会增加举报站内消息的选项, 随后被举报的消息会出现在版主控制面板上.',
	'ALLOW_QUICK_REPLY'			=> '允许快速回复',
	'ALLOW_QUICK_REPLY_EXPLAIN'	=> '关闭此选项将关闭整个论坛的快速回复, 当启用后由各个版面的设置决定其快速回复功能是否打开.',
...
$lang = array_merge($lang, array(
	...
	'ALLOW_REMOTE_UPLOAD_EXPLAIN'	=> '允许用户上传来自另一个网站的头像.',
	'ALLOW_UPLOAD'					=> '允许头像上传',
	'AVATAR_GALLERY_PATH'			=> '头像册路径',
	'AVATAR_GALLERY_PATH_EXPLAIN'	=> '在您的phpBB论坛根目录下的预先设置的头像画册, 例如 <samp>images/avatars/gallery</samp>',
	'AVATAR_STORAGE_PATH'			=> '头像存储路径',
	'AVATAR_STORAGE_PATH_EXPLAIN'	=> '在您的phpBB论坛根目录下的路径, 例如 <samp>images/avatars/upload</samp>。<br />如果路径不可写，头像上传将<strong>不可用</strong>。',
	'MAX_AVATAR_SIZE'				=> '最大头像尺寸',
	'MAX_AVATAR_SIZE_EXPLAIN'		=> '(用像素表示的宽 x 高)',
	'MAX_FILESIZE'					=> '最大的头像文件大小',
	'MAX_FILESIZE_EXPLAIN'			=> '针对上传的头像文件, 为0则仅受PHP设置的最大文件尺寸限制',
...
// Message Settings
$lang = array_merge($lang, array(
	'ACP_MESSAGE_SETTINGS_EXPLAIN'		=> '这里您可以设置站内消息的所有默认设置',

	'ALLOW_BBCODE_PM'			=> '在站内消息中允许BBCode',
	...
	'ALLOW_SMILIES_PM'			=> '允许在站内消息中使用表情图标',
	'BOXES_LIMIT'				=> '每个文件夹中的最大信件数量',
	'BOXES_LIMIT_EXPLAIN'		=> '用户每个文件夹中的消息将不能超过这个数量. 设置为0将不作限制.',
	'BOXES_MAX'					=> '每个用户的最大文件夹数量',
	'BOXES_MAX_EXPLAIN'			=> '默认情况下用户可以创建文件夹来存储自己的信件.',
	'ENABLE_PM_ICONS'			=> '在站内消息中允许使用主题图标',
...
$lang = array_merge($lang, array(
	...
	'BUMP_INTERVAL'					=> '顶帖间隔',
	'BUMP_INTERVAL_EXPLAIN'			=> '主题中最后一个帖子发表后的天数/小时数/分钟数, 设置为0则关闭此特性.',
	'CHAR_LIMIT'					=> '每个帖子的最大字数',
	'CHAR_LIMIT_EXPLAIN'			=> '设置为0则不作限制.',
	'DELETE_TIME'					=> '帖子删除时限',
	'DELETE_TIME_EXPLAIN'			=> '设置新帖子的可删除时间. 设置为0则不作限制.',
...
$lang = array_merge($lang, array(
	...
	'NEW_MEMBER_POST_LIMIT'			=> '新用户发帖限制',
	'NEW_MEMBER_POST_LIMIT_EXPLAIN'	=> '新用户会被归入 <em>新注册用户</em> 组直到其发表足够多的帖子. 您可以使用此群组的群组权限来限制其使用站内消息以及方便发帖审核. <strong>设置为0则不启用此项功能.</strong>',
	'NEW_MEMBER_GROUP_DEFAULT'		=> '设置新注册群组为默认组',
	'NEW_MEMBER_GROUP_DEFAULT_EXPLAIN'	=> '如果设置为是, 并且设置了新用户发帖数目限制, 新用户不仅会加入到 <em>新注册用户</em> 组, 这个群组也会成为他的默认组. 这将方便管理员为该组指定一个默认等级和组头像.',
...
$lang = array_merge($lang, array(
	...
	'ACP_FEED_GENERAL'					=> '综合设置',
	'ACP_FEED_POST_BASED'				=> '基于帖子的输出设置',
	'ACP_FEED_TOPIC_BASED'				=> '基于主题的输出设置',
	'ACP_FEED_SETTINGS_OTHER'			=> '其它设置',
...
$lang = array_merge($lang, array(
	...
	'CAPTCHA_GD_X_GRID_EXPLAIN'				=> '使用这种方式的较低设置生成基于GD的验证图片. 0 将禁用X轴的背景噪点.',
	'CAPTCHA_GD_Y_GRID'						=> 'GD 验证图片Y轴背景噪点',
	'CAPTCHA_GD_Y_GRID_EXPLAIN'				=> '使用这种方式的较低设置生成基于GD的验证图片. 0 将禁用Y轴的背景噪点.',
	...
	'CAPTCHA_PREVIEW_MSG'					=> '您的可视化验证设置尚未保存, 这只是预览.',
	'CAPTCHA_PREVIEW_EXPLAIN'				=> '您当前设置下的验证图片预览.',
	...
	'VISUAL_CONFIRM_REG'					=> '在用户注册时启用可视化验证',
	'VISUAL_CONFIRM_REG_EXPLAIN'			=> '在新用户注册时会被要求输入一组随机字符以防止机器人注册.',
...
// Cookie Settings
$lang = array_merge($lang, array(
	'ACP_COOKIE_SETTINGS_EXPLAIN'		=> '以下设置了送往用户浏览器的数据. 在大多数情况下使用默认设置就已足够. 如果您自行调整, 不正确的设置将会使用户无法登录论坛.',
...
$lang = array_merge($lang, array(
	...
	'LOAD_CPF_MEMBERLIST'			=> '允许界面在用户列表中显示自定义资料',
...
// Server Settings
$lang = array_merge($lang, array(
	'ACP_SERVER_SETTINGS_EXPLAIN'	=> '这里配置服务器和域名相关的设置. 请确保输入的数据是正确可靠的, 错误将导致email包含错误信息. 档输入域名时记住不包含http:// 和其他协议头. 只有当您的服务器使用一个特别的端口时才需要更改端口号, 一般使用的都是80.',
	...
	'SERVER_PROTOCOL_EXPLAIN'	=> '如果强制设置, 这将用于服务器协议. 如果留空或未强制设置, 协议由cookie安全设置决定 (<samp>http://</samp> or <samp>https://</samp>)',
	'SERVER_URL_SETTINGS'		=> '服务器URL设置',
...
// Security Settings
$lang = array_merge($lang, array(
	'ACP_SECURITY_SETTINGS_EXPLAIN'		=> '这里您可以进行对话和登录相关的设置',
	...
	'NO_REF_VALIDATION'				=> '无',
	'PASSWORD_TYPE'					=> '密码复杂度',
	'PASSWORD_TYPE_EXPLAIN'			=> '决定设置或更改时密码的复杂度, 多个选项则往前叠加.',
...
$lang = array_merge($lang, array(
	...
	'ENABLE_EMAIL_EXPLAIN'			=> '如果禁用, 论坛将不会发送任何email. <em>注意用户激活需要启用此项功能. 如果当前设置为用户自行激活和管理员激活, 则停用此项功能将使用户无需激活.</em>',
...
$lang = array_merge($lang, array(
	...
	'JAB_SETTINGS_CHANGED'		=> 'Jabber设置修改完成.',
	'JAB_USE_SSL'				=> '使用SSL连接',
